...
Td = SE3(0.3960, -0.1501, 0.2575)@SE3.Ry(np.pi/2)

t_total = np.arange(0, tfin, ts)
traj_des = ctraj(T, Td, t_total)
# print(traj_des)
...
def tau(p560nf, t, q, qd, VarGamma, Dd, Kp_t, Kv, Kp, F, t0, t1, t2, ts, Td):
    global t_ant, u, T, vel, pos, pos_T, k
    ...
    if (t > t_ant + ts):
        ...
        x_tilde = tr2delta_explicit(traj_des[k], pos_T)
...
tools.trajectory.qplot(tg.t, tg.q)
plt.show()
...
# Torques graph
tools.trajectory.tausplot(t_list, u_list)
plt.show()
